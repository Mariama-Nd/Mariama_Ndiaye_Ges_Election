...
<div class="container mx-auto p-4 border-4 border-orange-500 rounded-lg">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="text-center mb-4">
            <h1 class="text-2xl font-semibold text-gray-800">Liste des Programmes</h1>
        </div>
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            <a href="{{ route('programmes.create') }}" class="bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-gray-900">
                ...
                <tbody id="programmes-table">
                    @forelse($programmes as $programme)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td class="px-4 py-3">{{ $programme->nom }}</td>
                            <td class="px-4 py-3">{{ $programme->departement->nom }}</td>
                            <td class="px-4 py-3 flex items-center justify-center space-x-2">
                                <!-- Bouton Modifier -->
                                <a href="{{ route('programmes.edit', $programme) }}" class="btn-orange">
                                    Modifier
                                </a>
                                <!-- Bouton Supprimer -->
                                <form action="{{ route('programmes.destroy', $programme) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-red" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce programme ?')">
                                        Supprimer
                                    </button>
                                </form>
                            </td>
...
<script>
    document.getElementById('search-bar').addEventListener('input', function() {
        ...
        const rows = document.querySelectorAll('#programmes-table tr');
