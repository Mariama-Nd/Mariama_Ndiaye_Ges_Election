...
<div class="container mx-auto p-4 border-4 border-orange-500 rounded-lg">
    <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="text-center mb-4">
            <h1 class="text-2xl font-semibold text-gray-800">Liste des Étudiants</h1>
        </div>
                ...
                <tbody id="etudiants-table">
                    @forelse($etudiants as $etudiant)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td class="px-4 py-3">{{ $etudiant->matricule }}</td>
                            <td class="px-4 py-3">{{ $etudiant->nom }}</td>
                            <td class="px-4 py-3">{{ $etudiant->prenom }}</td>
                            <td class="px-4 py-3">{{ $etudiant->programme->nom }}</td>
                            <td class="px-4 py-3 flex items-center justify-center space-x-2">
                                <!-- Bouton Modifier -->
                                <a href="{{ route('etudiants.edit', $etudiant) }}" class="btn-orange">
                                    Modifier
                                </a>
                                <!-- Bouton Supprimer -->
                                <form action="{{ route('etudiants.destroy', $etudiant) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-red" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet étudiant ?')">
                                        Supprimer
                                    </button>
                                </form>
                            </td>
...
<script>
    document.getElementById('search-bar').addEventListener('input', function() {
        ...
        const rows = document.querySelectorAll('#etudiants-table tr');
